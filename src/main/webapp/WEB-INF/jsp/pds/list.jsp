<%@ page pageEncoding="UTF-8" %>


<div id="main">
        <div class="margin30">
            <h1><i class="fa fa-upload fa-2x"> 자료실</i></h1>
            <hr>
        </div> <!--타이틀-->


        <div class="row margin1050">
            <div class="col-12 text-right">
                <button type="button" id="newbd"
                        class="btn btn-light">
                    <i class="fa fa-plus-circle"></i>
                    이전게시물
                </button>
            </div>
        </div><!-- 버튼들-->

        <div class="row margin1050">
            <div class="col-12">
                <table class="table table-striped"
                    style="border-bottom:3px solid black;
                           border-top:3px solid black">
                    <thead style="background: #dff0d8">
                        <tr><th style="width: 7%" class="text-center">번호</th>
                            <th>제목</th>
                            <th style="width: 12%">작성자</th>
                            <th style="width: 10%">작성일</th>
                            <th style="width: 7%">추천</th>
                            <th style="width: 7%">조회</th></tr>
                    </thead>
                    <tbody>
                        <tr><th>공지</th>
                            <th><span class="badge badge-danger">Hot</span>
                            '다크나이트 라이지즈' 예매권 증정이벤트 실시!!</th>
                            <th>운영자</th>
                            <th>2012.07.15</th>
                            <th>10</th>
                            <th>128</th></tr>
                        <tr><td>1</td>
                            <td><a href="pds/view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>2</td>
                            <td><a href="pds/view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>3</td>
                            <td><a href="pds/view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>4</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a>
                                <span class="badge badge-warning">Hot</span></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>5</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>6</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>7</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>8</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>9</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>10</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>11</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>12</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>13</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>14</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>15</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>16</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>17</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>18</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>19</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>20</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>21</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>22</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>23</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>24</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>
                        <tr><td>25</td>
                            <td><a href="view.html">장바구니 맨 시의원 처음 보셨나요?</a></td>
                            <td>902호 달인</td>
                            <td>2012.07.15</td>
                            <td>10</td>
                            <td>128</td></tr>

                    </tbody>
                </table>
            </div>
        </div><!-- 게시판 목록-->
        <div class="row margin1050">
            <div class="col-12">
                <nav>
                    <ul class="pagination justify-content-center">
                        <li class="page-item disabled">
                            <a href="#" class="page-link">이전</a></li>
                        <li class="page-item active">
                            <a href="#" class="page-link">1</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">2</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">3</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">4</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">5</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">6</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">7</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">8</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">9</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">10</a></li>
                        <li class="page-item ">
                            <a href="#" class="page-link">다음</a></li>

                    </ul>
                </nav>
            </div>
        </div><!-- 페이지 네비게이션-->
    </div><!--메인영역 끝-->
